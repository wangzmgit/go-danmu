...
func CheckFile(Filename string) bool {
	...
	if _, err := os.Stat(Filename); os.IsNotExist(err) {
		exist = false
		if err != nil {
			PrintLog("[Info]", " No log file found")
		}
	}
	return exist
}
...
func Logfile(logType string, log string) {
	...
	if CheckFile(filenames) { //如果文件存在
		...
		if err != nil {
			PrintLog("[Error]", " Log file opening failed "+err.Error())
			return
		}
	} else {
		f1, err = os.Create(filenames) //创建文件
		if err != nil {
			PrintLog("[Error]", " Log file creation failed "+err.Error())
			return
		}
	}
	_, err = io.WriteString(f1, logType+time.Now().Format("2006-01-02 15:04:05")+log+"\n") //写入文件
	if err != nil {
		PrintLog("[Error]", " Write log error "+err.Error())
	}
}
